...
def test_transform_dict_with_nested_link_to_cbor():
    ...
    expected = {
        ...
        "hello": "world",
        "l1": Tag(LINK_TAG, "takemedowntotheparadisecity"),
        "secret": {"l1": Tag(LINK_TAG, "Ihhh ein Sekret!")},
    }
...
def test_transform_cbor_with_link_to_dict():
    src = dumps(
        {"num": 1, "hello": "world", "l1": Tag(LINK_TAG, "/p2p/QmaozNR7DZHQK1ZcU9p7QdrshMvXqWK6gpu5rmrkPdT3L4")},
        sort_keys=True,
    )
    ...
    expected = {
        "hello": "world",
        "num": 1,
        "l1": {"/": "/p2p/QmaozNR7DZHQK1ZcU9p7QdrshMvXqWK6gpu5rmrkPdT3L4"},
    }
...
def test_transform_cbor_with_faulty_link_to_dict():
    src = dumps({"num": 1, "hello": "world", "l1": Tag(LINK_TAG, "takemedowntotheparadisecity")}, sort_keys=True)
    ...
    src = dumps(
        {
            ...
            "secret": {"l1": Tag(LINK_TAG, "/p2p/QmaozNR7DZHQK1ZcU9p7QdrshMvXqWK6gpu5rmrkPdT3L4")},
        },
        sort_keys=True,
    )
...
def test_multihashing_cbor():
    src = dumps({"name": "hello.txt", "size": 11}, sort_keys=True)
